...
def _configure_attractor_animation(fig, trajectory, dims=3, frame_duration_ms=55, frame_cap=240, marker_name="Punto móvil"):
    ...
    fig.update_layout(
        uirevision="attractor_camera_live_v15511",
        scene=dict(uirevision="attractor_camera_live_v15511") if int(dims) >= 3 else None,
        updatemenus=[dict(
...
def _render_live_attractor_3d(fig, tracks, frame_duration_ms=55, frame_cap=180, height=620):
    """
    v15.5.11 · Animación 3D realmente interactiva.
    ...
    (function() {{
      ...
      // Importante: NO hay Plotly.animate(), Plotly.react() ni relayout() durante
      // la reproducción. Sólo se actualizan x/y/z del marcador. La cámara queda libre.
      drawFrame(0);
    }})();
    </script>
    """
    ...
    for rec in list(records or []):
        ...
        if m_rec >= 3:
            ...
            fig3.update_layout(title=f"Atractor reconstruido 3D · {rec}", height=650,
                               scene=dict(xaxis_title="RR(t)", yaxis_title="RR(t-τ)", zaxis_title="RR(t-2τ)", uirevision="attractor_camera_live_v15511"),
                               showlegend=False)
    ...
    if _attr_records:
        ...
        with _anim_c1:
            _animate_attr = st.toggle(
                "▶ Animar trayectoria", value=False, key="attractor_animate_v15511",
                help="Mantiene visible la trayectoria completa y hace avanzar un punto móvil siguiendo el orden temporal del embedding."
            )
        with _anim_c2:
            _anim_speed_label = st.selectbox(
                "Velocidad", ["Lenta", "Media", "Rápida"], index=1, key="attractor_anim_speed_v15511",
                disabled=not _animate_attr
            )
        with _anim_c3:
            _anim_frames = st.select_slider(
                "Suavidad (frames)", options=[60,120,180,240], value=180, key="attractor_anim_frames_v15511",
        ...
        if _animate_attr:
            st.caption("La línea conserva todo el atractor y el punto móvil recorre cronológicamente sus estados. En 3D la animación es cliente-side: puedes rotar, hacer zoom y desplazar la cámara incluso mientras está en reproducción, sin que vuelva a la posición inicial.")
        ...
        _valid_attr = 0
        _attr_cache_v15323 = {}
        _anim_tracks3_v15511 = []
        _anim_tracks2_v15511 = []
        for _rec in _attr_records:
            ...
            _anim_tracks2_v15511.append((_short_record_label(_rec, 28), _show))
            if _m_rec >= 3:
                _anim_tracks3_v15511.append((_short_record_label(_rec, 28), _show))
                ...
                if any((v.get("ok") and int(v.get("m", 0)) >= 3) for v in _attr_cache_v15323.values()):
                    ...
                    if _animate_attr:
                        _render_live_attractor_3d(
                            _fig3, _anim_tracks3_v15511,
                            frame_duration_ms=_anim_speed_ms,
                            frame_cap=int(_anim_frames),
                            height=680
                        )
                    else:
                        st.plotly_chart(_fig3, use_container_width=True, key="attractor_compare_3d_v15511")
                if _animate_attr:
                    _fig2 = _configure_multi_attractor_animation(_fig2, _anim_tracks2_v15511, dims=2, frame_duration_ms=_anim_speed_ms, frame_cap=int(_anim_frames))
                _fig2.update_layout(
                        ...
                        for _ci, _pid in enumerate(_attr_pids):
                            ...
                            with _cols[_ci]:
                                ...
                                if _m_rec>=3:
                                    ...
                                    _fi.update_layout(height=500,scene=dict(xaxis_title="RR(t)",yaxis_title="RR(t-τ)",zaxis_title="RR(t-2τ)",uirevision="attractor_camera_live_v15511"),showlegend=False,margin=dict(l=0,r=0,t=20,b=0))
                                else:
                                ...
                                if _animate_attr and _m_rec >= 3:
                                    ...
                                    st.plotly_chart(_fi,use_container_width=True,key=f"attr_pat_{hashlib.md5((str(_pid)+'|'+str(_rec)).encode()).hexdigest()[:12]}_v15511")
                else:
                        ...
                        for _j, _rec in enumerate(_attr_records_visible[_i:_i+_ncols]):
                            with _cols[_j]:
                                ...
                                if _m_rec>=3:
                                    ...
                                    _fi.update_layout(height=500 if _ncols == 1 else 430,scene=dict(xaxis_title="RR(t)",yaxis_title="RR(t-τ)",zaxis_title="RR(t-2τ)",uirevision="attractor_camera_live_v15511"),showlegend=False)
                                else:
                                ...
                                if _animate_attr and _m_rec >= 3:
                                    ...
                                    if _animate_attr:
                                        _fi = _configure_attractor_animation(_fi, _show, dims=2, frame_duration_ms=_anim_speed_ms, frame_cap=int(_anim_frames))
                                    st.plotly_chart(_fi,use_container_width=True,key=f"attr_ind_{hashlib.md5(str(_rec).encode()).hexdigest()[:10]}_v15511")
